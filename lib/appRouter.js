...
function appRouter(opts) {
	...
	router.get('/finish_auth', session, function(req, res) {
		...
		var doNext = (req.session.shopConfig ? 
			...
			Promise.resolve(validateRequest(req)).then(function(isValid){
		...
		doNext.then(function(shopConfig){
	...
	router.post('/uninstall', function(req, res) {
		...
		shop = shop.split('.')[0];
		req.db.func('uninstall_shop_token', [appName, shop], 1).then(function(result) {
			if (opts.onUninstall) {
