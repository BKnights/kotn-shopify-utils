...
function appRouter(opts) {
    ...
    var session = opts.sessionHandler || (function(){ // needs session for auth process so supply simplest

        var session = require('express-session')({
            secret: process.env.COOKIE_SECRET || (Math.floor(new Date().getTime()*Math.random()) +'::'+ new Date().getTime()),
            resave: false,
    ...
    router.use(function(req, res, next){
        res.locals.ctx = {
            apiKey : apiKey,
            shop: req.session && req.session.shopConfig ? req.session.shopConfig.shop : null
        };
        next();
    });
    ...
    router.get('/', session, function(req, res) {
        ...
        res.render('pages' + installPath + '/index', res.locals.ctx);
    });
    ...
    function getShopConfig(req, nosession) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            lookupShopConfig(req.db, appName, shop).then(function(freshConfig) {
                if (!nosession) req.session.shopConfig = freshConfig;
                resolve(freshConfig);
            }, function(err) {
                console.error(err || 'no installed shop');
                reject(err || 'no installed shop');
            });
        });
    }

    function lookupShopConfig(db, appName, shopName) {
        return db.one("select token from app_installs where app = $1 and shop = $2 " + "and uninstalled is null", [appName, shopName]).
        then(function(row) {
    ...
    router.get("/preferences", session, function(req, res) {
        getShopConfig(req).then(function(config) {
            res.render('pages' + installPath + '/prefs', res.locals.ctx);
        }, function(err) {
