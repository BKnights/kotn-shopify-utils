...
function appRouter(opts) {
    ...
    function lookupShopConfig(db, appName, shopName) {
        return db.one("select token, is_subscribed, subscription_ends from app_installs where app = $1 and shop = $2 " + "and uninstalled is null", [appName, shopName]).
        then(function(row) {
    ...
    router.get("/preferences", session, function(req, res) {
        getShopConfig(req).then(function(config) {
            res.render('pages' + installPath + '/prefs', Object.extend({is_subscribed: config.is_subscribed, subscription_ends:config.subscription_ends}, res.locals.ctx));
        }, function(err) {
