...
function appRouter(opts) {
	...
	var backoff_level = opts.shopifyBackoffLevel || 0.8;
	...
	function processAuth(shop, req, res) {
		...
		if(req.query.checkSession == 'T' || res.locals.doScriptRedirect){
	...
	router.get('/finish_auth', session, function(req, res) {
		...
		var doNext = (req.session.shopConfig ? 
			...
			then(function(isValid){
				...
				processAuth(req.query.shop.split('.')[0], req, res);
				return null;
			}));
		doNext.
		then(function(shopConfig){
			...
			return new Promise(function(resolve, reject){
				...
				return req.db.func('get_shop_token', [appName, Shopify.config.shop.split('.')[0], token], 1).
