...
const debug = require('debug')('kotn-base');
...
function appRouter(opts) {
	...
	function processAuth(shop, req, res) {
		...
		var withRedirect = (req.query.checkSession == 'T' || res.locals.doScriptRedirect) ? Promise.resolve(true) :
		(()=>{
			return req.db.func('check_shop_access', [appName, Shopify.config.shop.split('.')[0], scopeHash], 1).
				then(result=>{
	...
	router.get('/finish_auth', session, function(req, res) {
		...
		var doNext = (req.session.shopConfig ? 
			...
			Promise.resolve(validateRequest(req)).
		...
		then(function(shopConfig){
			...
			}).then(function(token){ // have a valid token 
				return req.db.func('get_shop_token', [appName, Shopify.config.shop.split('.')[0], token, scopeHash], 1).
	...
	function lookupShopConfig(db, appName, shopName, defaults) {
		return db.one('select id, token, is_subscribed, subscription_ends, scope_hash from app_installs where app = $1 and shop = $2 and uninstalled is null', [appName, shopName]).
		then(function(row) {
	...
	function validSession(req, res, next){
		...
		if(!config){
			...
			res.redirect(installPath + '/auth?checkSession=T');
			return;
		}
