					...
					then((result)=>{
						...
						if (result.isnew) {
							return new Promise((resolve, reject)=>{
								...
								Shopify.post('/admin/webhooks.json', uninstallHook, (err, data)=>{
									if (err) {
										reject(err);
										return;
									}
	...
	router.post('/uninstall', (req, res)=>{
		...
		return uninstall.then(()=>{
			return lookupShopConfig(req.db, appName, shop).then(shopConfig=>{
				...
				return shopPromise.get(shopConfig, '/admin/webhooks.json').then((data)=> {
					...
					const removeHook = ()=> {
						...
						return shopPromise.delete(shopConfig, '/admin/webhooks/' + hook.id + '.json').then(()=> {
							processed.push(hook);
							return removeHook();
						});
					};
