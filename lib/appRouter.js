...
function appRouter(opts) {
	...
	function validateRequest(req){
		...
		if(!digestMatch) console.error('invalid digest '+( haveHmac ? 'with' : 'without') +' hmac from '+parts.join('&'));
		return digestMatch;
	...
	function validSession(req, res, next){
		...
		if(!config){
			console.log('invalid prefs access');
			res.redirect(installPath + '/auth');
			return;
		}
		console.log('already valid prefs access');
